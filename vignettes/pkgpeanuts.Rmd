...
this means that `pour` was unable to create a symlink due to restrcited permissions. You need to run RStudio as administrator to avoid this warning. The symbolic link is necessary for the current approach to combining `lintr` with `testthat` unit testing and not failing a Travis-CI build. I think there is now another way to do this, but I have not looked into it yet. Stay tuned.
